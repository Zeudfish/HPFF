...
import networks.resnetInfoPro_HPFF
...
record_path = './' \
              + ('InfoPro*_HPFF_' if args.balanced_memory else 'InfoPro_HPFF_') \
              + str(args.dataset) \
...
def main():
    ...
    if args.model == 'resnet':
        model = eval('networks.resnetInfoPro_HPFF.resnet' + str(args.layers))\
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (x, target) in enumerate(tqdm(train_loader)):
        ...
        optimizer.zero_grad()
        output_x,output_y, loss = model(img=x,
                             target=target,
                             ixx_1=args.ixx_1,
                             ixy_1=args.ixy_1,
                             ixx_2=args.ixx_2,
